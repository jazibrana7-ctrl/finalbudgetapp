...
def fetch_petrol_price():
    """Best-effort to grab petrol price from web"""
    try:
        import requests
        r = requests.get(
...
with tab4:
    ...
    with col1:
        fuel_price = st.number_input(
            "Fuel Price (Rs/Liter):",
            value=float(DATA["transport"]["fuel_price"]),
            min_value=0.0,
            step=1.0,
            key="fuel_price"
        )
    ...
    with col2:
        if st.button("🔄 Auto-Fetch Petrol Price"):
            ...
            if fetched:
                DATA["transport"]["fuel_price"] = fetched
                st.success(f"✅ Updated to Rs. {fetched:.2f}/L")
                st.rerun()
            else:
    ...
    with col1:
        civic = st.number_input(
            "Civic (Liters/month):",
            value=float(DATA["transport"]["civic_liters"]),
            min_value=0.0,
            step=5,
            key="civic"
        )
    ...
    with col2:
        cultus = st.number_input(
            "Cultus (Liters/month):",
            value=float(DATA["transport"]["cultus_liters"]),
            min_value=0.0,
            step=5,
            key="cultus"
        )
    ...
    with col3:
        bike = st.number_input(
            "Bike (Liters/month):",
            value=float(DATA["transport"]["bike_liters"]),
            min_value=0.0,
            step=5,
            key="bike"
        )
    ...
    with col3:
        if st.button("➕ Add", use_container_width=True):
            if new_name.strip():
                ...
                DATA["custom_expenses"][custom_key] = (new_name, new_amount)
...
with tab5:
    ...
    if st.button("🔢 CALCULATE TOTAL", use_container_width=True, type="primary"):
        ...
        # Calculate split
        jazib_share = DATA["last_jazib_share"]
        ...
        with col2:
            ...
            <div class="stat-box stat-box-jazib">
                ...
                <div class="stat-value">Rs. {jazib_share:,.0f}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with stat_col2:
            st.metric("Your Payment", f"Rs. {jazib_share:,.0f}")
        with stat_col3:
            percentage = (jazib_share / total * 100) if total > 0 else 0
